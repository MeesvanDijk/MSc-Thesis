...
window = 10;                                                         %time window of a single sample in seconds
overlap = 8;                                                        %overlap of the time windows
...
for i = 1:repetitions
    for patient = 1:length(sample_patients)
        ...
        for R = 1:R_range
            dimension = size(Xq{1});
            X_TT = TTND(Xq,R,epsilon);
            X_tTT = TTND(X_qt,R, epsilon);
...
%%
file_name_loso = "/Users/meesvandijk/Documents/MEES2/MEES/Systems & Control/Msc Thesis/EEG_MATLAB/Results/E3_TTMMK_LOSO2.mat"
...
overlap = 7;                                                        %overlap of the time windows
...
for i = 1:repetitions
    ...
    for R = 1:R_range
        dimension = size(Xq1{1});
        X_TT1 = TTND(Xq1,R,epsilon);
        X_tTT1 = TTND(X_qt1,R, epsilon);
        X1_CP = ttcptensor(X_TT1,R,R,dimension,truncation);
        X_t1_CP = ttcptensor(X_tTT1, R, R, dimension, truncation);

        X_TT3 = TTND(Xq3,R,epsilon);
        X_tTT3 = TTND(X_qt3,R, epsilon);
        X3_CP = ttcptensor(X_TT3,R,R,dimension,truncation);
        X_t3_CP = ttcptensor(X_tTT3, R, R, dimension, truncation);

        X_TT8 = TTND(Xq8,R,epsilon);
        X_tTT8 = TTND(X_qt8,R, epsilon);
        X8_CP = ttcptensor(X_TT8,R,R,dimension,truncation);
        X_t8_CP = ttcptensor(X_tTT8, R, R, dimension, truncation);
        ...
        R_tot = R*R
        % clear X1 X_t1 y1 y1_t1 X3 X_t3 y3 y_t3 X8 X_t8 y8 y_t8 X1_CP X3_CP X8_CP X_t1_CP X_t3_CP X_t8_CP
        [scores, preds, all_y_test, all_probs, copt, gopt] = LOPO(X_CP, X_CP_t, y, y_t, div, R_tot, flagker, g, C, kfold, i, store_C, store_g);
        store_C(R,:) = copt
        store_g(R,:) = gopt
...
file_name_lopo = "/Users/meesvandijk/Documents/MEES2/MEES/Systems & Control/Msc Thesis/EEG_MATLAB/Results/E3_TTMMK_LOPO2.mat"
...
xticks(ticks)
ylim([0 1])
xlabel('TT-Rank', 'Interpreter','Latex')
ylabel('F1-score', 'Interpreter','latex')
title('F1-score SHTM LOSO', 'Interpreter', 'latex')
...
title('Accuracy TT-MMK LOSO', 'Interpreter', 'latex')
legend('chb01', '', 'chb03', '', 'chb08')
...
xlabel('CP-Rank', 'Interpreter','Latex')
ylabel('F1-score', 'Interpreter','latex')
title('F1-score SHTM LOPO', 'Interpreter', 'latex')
...
xlabel('CP-Rank', 'Interpreter','Latex')
ylabel('Accuracy [\%]', 'Interpreter','latex')
title('Accuracy SHTM LOPO', 'Interpreter', 'latex')